...
export default class NeonBreakoutScene extends Phaser.Scene {
  ...
  create(data: { difficulty?: string; mode?: ArcadeMode }) {
    ...
    this.add.rectangle(320, 250, 590, 374, 0x061827, 0.82).setStrokeStyle(2, 0x00ffcc, 0.35);
  ...
  private createTextures() { const make = (key: string, width: number, height: number, draw: (graphics: Phaser.GameObjects.Graphics) => void) => { if (this.textures.exists(key)) return; const graphics = this.add.graphics(); draw(graphics); graphics.generateTexture(key, width, height); graphics.destroy(); }; make('neon-paddle', 108, 16, g => { g.fillStyle(0x063d4a).fillRoundedRect(0, 0, 108, 16, 7); g.fillStyle(0x9ffff1).fillRoundedRect(8, 2, 92, 5, 3); g.lineStyle(2, 0x00ffcc).strokeRoundedRect(0, 0, 108, 16, 7); }); make('neon-ball', 14, 14, g => { g.fillStyle(0x00ffcc, .28).fillCircle(7, 7, 7); g.fillStyle(0xffffff).fillCircle(7, 7, 4); }); make('neon-brick', 58, 18, g => { g.fillStyle(0xffffff).fillRoundedRect(0, 0, 58, 18, 3); g.fillStyle(0x07121c, .4).fillRect(3, 11, 52, 4); g.lineStyle(1, 0xffffff, .8).strokeRoundedRect(0, 0, 58, 18, 3); }); make('neon-drop', 18, 18, g => g.lineStyle(2, 0xffffff).strokeCircle(9, 9, 7).lineBetween(4, 9, 14, 9)); make('neon-laser', 4, 16, g => g.fillStyle(0xffffff).fillRect(0, 0, 4, 16)); }
}
