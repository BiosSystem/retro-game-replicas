...
export async function signModManifest(manifest: unknown, privateKey?: CryptoKey) { const validation = validateModManifest(manifest); if (!validation.valid || !validation.manifest) throw new Error(validation.errors.join('; ')); let signingKey = privateKey; let publicKey: CryptoKey; if (signingKey) { const exported = await crypto.subtle.exportKey('jwk', signingKey); if (!exported.x) throw new Error('Signing key has no public coordinate'); publicKey = await crypto.subtle.importKey('jwk', { kty: 'OKP', crv: 'Ed25519', x: exported.x, ext: true, key_ops: ['verify'] }, { name: 'Ed25519' }, true, ['verify']); } else { const pair = await crypto.subtle.generateKey({ name: 'Ed25519' }, true, ['sign', 'verify']) as CryptoKeyPair; signingKey = pair.privateKey; publicKey = pair.publicKey; } const bytes = canonicalBytes(validation.manifest); const signature = await crypto.subtle.sign({ name: 'Ed25519' }, signingKey, bytes); const rawPublic = await crypto.subtle.exportKey('raw', publicKey); return { package: { version: 1 as const, manifest: validation.manifest, sha256: await hashManifest(validation.manifest), publicKey: toBase64(new Uint8Array(rawPublic)), signature: toBase64(new Uint8Array(signature)) }, privateKey: signingKey }; }
