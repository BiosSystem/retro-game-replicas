...
export default class CartridgePlayerScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.input.keyboard?.on('keydown-ENTER', () => void this.loadBuiltin()); this.input.keyboard?.on('keydown-L', () => this.openFilePicker()); this.input.keyboard?.on('keydown-ESC', () => this.scene.start('LobbyScene'));
  ...
  async loadBuiltin() { const { NEON_INVADER_CARTRIDGE } = await import('../../core/scripting/demo/neon-invader.cart'); this.loadCartridge(NEON_INVADER_CARTRIDGE); }
  loadCartridge(source: ArrayBuffer | Uint8Array) {
    try {
      ...
      this.vm = new NeonBytecodeVM(this.cartridge.bytecode, bridge); this.vm.writeHeapInt32(DEMO_STATE.playerX, 320); this.vm.writeHeapInt32(DEMO_STATE.score, 0); this.vm.writeHeapInt32(DEMO_STATE.lives, 3); this.errorText?.destroy(); this.errorText = undefined; this.title.setText(`${this.cartridge.metadata.title} // ${this.cartridge.metadata.author}`);
  ...
  private renderStatus() { if (!this.vm) return; const score = this.vm.readHeapInt32(DEMO_STATE.score); const lives = this.vm.readHeapInt32(DEMO_STATE.lives); this.status.setText(`VM ${this.lastInstructions.toString().padStart(4, '0')} / ${FRAME_QUOTA} OPS   HEAP 12 B LIVE / 1 MiB BOUND   SCORE ${score.toString().padStart(5, '0')}   LIVES ${lives}`); }
