...
export class NetplayController {
  ...
  private createPeer() { const peer = new PeerLink(loadIceConfiguration()); this.mesh.add(peer); peer.addEventListener('status', event => this.status((event as CustomEvent<PeerStatus>).detail)); peer.addEventListener('input', event => { const input = (event as CustomEvent<NetInputFrame>).detail; InputManager.setNetworkPlayerState({ UP: Boolean(input.buttons & 1), DOWN: Boolean(input.buttons & 2), LEFT: Boolean(input.buttons & 4), RIGHT: Boolean(input.buttons & 8), FIRE: Boolean(input.buttons & 16) }); }); peer.addEventListener('control', event => { this.mesh.receive((event as CustomEvent).detail); }); return peer; }
