...
export class NetplayController {
  private readonly root: HTMLElement; private readonly peer = new PeerLink(loadIceConfiguration());
  constructor() {
    this.root = document.createElement('aside'); this.root.className = 'netplay-panel'; this.root.hidden = true;
    this.root.innerHTML = `<header><div><small>BIOSSYSTEM DIRECT LINK</small><h2>P2P NETPLAY</h2></div><button type="button" data-close>X</button></header><p>Exchange room codes through a trusted channel. Configure TURN outside the bundle when relay service is required.</p><div class="netplay-actions"><button type="button" data-host>CREATE HOST CODE</button><button type="button" data-join>ACCEPT HOST CODE</button><button type="button" data-answer>ACCEPT ANSWER</button></div><textarea data-code spellcheck="false" aria-label="WebRTC room code" placeholder="Paste ARC1 room code"></textarea><output data-status>IDLE</output>`;
    document.body.appendChild(this.root); this.bind(); requestAnimationFrame(() => this.tick());
  }
  ...
  private bind() {
    ...
    this.peer.addEventListener('status', event => this.status((event as CustomEvent<PeerStatus>).detail));
    this.peer.addEventListener('input', event => { const input = (event as CustomEvent<NetInputFrame>).detail; InputManager.setNetworkPlayerState({ UP: Boolean(input.buttons & 1), DOWN: Boolean(input.buttons & 2), LEFT: Boolean(input.buttons & 4), RIGHT: Boolean(input.buttons & 8), FIRE: Boolean(input.buttons & 16) }); });
  ...
  private frame = 0;
  private tick() { if (this.peer.getStatus() === 'CONNECTED') { const buttons = (InputManager.isP1Down('UP') ? 1 : 0) | (InputManager.isP1Down('DOWN') ? 2 : 0) | (InputManager.isP1Down('LEFT') ? 4 : 0) | (InputManager.isP1Down('RIGHT') ? 8 : 0) | (InputManager.isP1Down('FIRE') ? 16 : 0); const frame = ++this.frame; const axisX = buttons & 4 ? -127 : buttons & 8 ? 127 : 0; const axisY = buttons & 1 ? -127 : buttons & 2 ? 127 : 0; this.peer.sendInput({ frame, buttons, axisX, axisY, checksum: inputChecksum(frame, buttons, axisX, axisY) }); } requestAnimationFrame(() => this.tick()); }
