...
export class NetplayController {
  ...
  private readonly peers: PeerLink[] = []; private readonly gossip = new ScoreGossip(); private readonly identity = createSwarmIdentity(); private readonly scoreStore = new SwarmScoreStore(); private readonly voice = new SpatialVoiceMixer(); private localVoice?: MediaStream; private listener = { x: 320, y: 240, angle: 0 };
  constructor() {
  ...
  private bind() {
    ...
    window.addEventListener('arcade-hub-local', event => { const detail = (event as CustomEvent).detail as { x?: unknown; y?: unknown; angle?: unknown }; if ([detail.x, detail.y, detail.angle].every(Number.isFinite)) { this.hubState = { x: Number(detail.x), y: Number(detail.y), angle: Number(detail.angle) }; this.listener = { ...this.hubState }; } });
