...
export function encodeCartridge(cartridge: NeonCartridge): ArrayBuffer {
  ...
  const encoder = new TextEncoder(); const title = encoder.encode(cartridge.metadata.title); const author = encoder.encode(cartridge.metadata.author); const version = encoder.encode(cartridge.metadata.version);
...
export function decodeCartridge(source: ArrayBuffer | Uint8Array): NeonCartridge {
  const bytes = source instanceof Uint8Array ? source : new Uint8Array(source); const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  try {
    ...
    const strings = titleLength + authorLength + versionLength; requireBytes(bytes, offset, strings + bytecodeLength + 4); const decoder = new TextDecoder(); const title = decoder.decode(bytes.subarray(offset, offset += titleLength)); const author = decoder.decode(bytes.subarray(offset, offset += authorLength)); const version = decoder.decode(bytes.subarray(offset, offset += versionLength)); const bytecode = bytes.subarray(offset, offset += bytecodeLength);
...
function requireBytes(bytes: Uint8Array, offset: number, length: number) { if (offset < 0 || length < 0 || offset + length > bytes.length) throw new CartridgeCodecError(); }
