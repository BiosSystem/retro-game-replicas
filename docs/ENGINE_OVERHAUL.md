...
Read menu input from the shared normalized controller frame. Map D-pad or left-stick up and down to navigation, the south face button to confirm, and the east face button or Select to back. Apply rising-edge checks in overlays so a held stick or button does not repeat a command. Reserve Start for pausing active games and opening Cabinet Control from the lobby. Pause the pause menu before opening Cabinet Control so only the foreground overlay consumes input.
