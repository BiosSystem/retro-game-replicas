...
Read menu input from the shared normalized controller frame. Map D-pad or left-stick up and down to navigation, the south face button to confirm, and the east face button or Select to back. Map the horizontal axis to high-score initial-slot movement while up and down change the selected character. Apply rising-edge checks in overlays so a held stick or button does not repeat a command. Suspend the legacy bridge for Name Entry as well as Pause and Cabinet Control. Reserve Start for pausing active games and opening Cabinet Control from the lobby. Pause the pause menu before opening Cabinet Control so only the foreground overlay consumes input.
