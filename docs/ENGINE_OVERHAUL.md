...
Keep keyboard-owned replicas compatible through an opt-in scene bridge. While Snake, Tetris, Frogger, Brave Bird, Cyber Chasm, or Neon Singularity is active, map the first normalized controller's stick or D-pad directions and face buttons to its existing arrow and Space handlers. Emit only state transitions and release every synthetic key when the scene shuts down. Track each active bridge owner independently, then suspend all synthetic keys while Pause or Cabinet Control is active and restore eligible owners when the overlay closes. Keep the lobby and shared-input scenes on their native controller paths to avoid duplicate commands.

Read menu input from the shared normalized controller frame. Map D-pad or left-stick up and down to navigation, the south face button to confirm, the north face button to open Achievements from the lobby, and the east face button or Select to back. Map the horizontal axis to high-score initial-slot movement while up and down change the selected character. Capture keyboard Escape before scene-local handlers. If a visible DOM utility panel has `data-arcade-overlay`, hide it and stop propagation before gameplay sees Escape. Otherwise open shared Pause from active gameplay, then let Pause, Game Over, Name Entry, Settings, and Achievements keep their own Escape behavior. Block controller Start pause input while a marked DOM utility panel is visible. Route every active classic loss path through one Game Over overlay that pauses the source, handles pointer and keyboard fallback, routes a new high score to Name Entry, restarts the original scene with south or Space, or stops the source and returns to the lobby with east, Select, or Escape. Copy the paused source scene's launch payload before applying Pause Restart so difficulty and arcade mode remain stable. Apply rising-edge checks in overlays so a held stick or button does not repeat a command. Suspend the legacy bridge for Name Entry, Game Over, Pause, and Cabinet Control. Reserve Start for pausing active games and opening Cabinet Control from the lobby. Exclude foreground overlays from global Start pause interception. Pause the pause menu before opening Cabinet Control so only the foreground overlay consumes input.
...
Preserve valid high-refresh deltas instead of forcing every update to 16.67 ms. Accept 8.33 ms at 120 Hz, 6.94 ms at 144 Hz, and variable positive VRR intervals unchanged. Reject non-finite or non-positive deltas and cap long gaps at 50 ms. Install `PhaserDeltaGuard` in a microtask after Phaser emits its game-ready event and binds the real `Game.step` callback, then pass the bounded value into every active scene, timer, tween, and physics update. Restore the original callback during guard teardown. Keep the standalone fixed-step loop capped at four updates per animation frame and bound its remaining accumulator time after catch-up saturation.

Publish `data-frame-delta-cap="50ms"` on the document root as a browser integration diagnostic. Keep the guard out of Phaser internals and avoid patching dependency files. Count cap events and the largest observed raw delta in an allocation-free snapshot for runtime inspection.
