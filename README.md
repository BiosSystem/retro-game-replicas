...
- **Reliable Pause Restart** - Use Escape or controller Start to pause an active game, use Escape to close an open utility panel first, quit only through the explicit Pause menu action, and restart without losing the active difficulty, local co-op mode, or versus mode configuration.
