...
- **Complete Controller Navigation** - Poll connected controllers once per runtime frame, normalize standard PlayStation and Xbox layouts into compact bitmasks, expose press and release edges, apply radial dual-stick deadzones, and support D-pad or stick menu navigation, controller-only high-score initials, controller Game Over restart, south-button selection, north-button Achievements access, east-button or Select back, and Start-to-pause without duplicate scene polling. Bridge Player 1 directions plus face buttons to the six keyboard-owned replicas only while each scene is active.
