...
- **Cabinet Fullscreen Control** - Enter or exit browser fullscreen from Cabinet Control only after an explicit player action, then retain a safe unavailable or denied fallback without a dependency.
- **Complete Controller Navigation** - Poll connected controllers once per runtime frame, normalize standard PlayStation and Xbox layouts into compact bitmasks, expose press and release edges, apply radial dual-stick deadzones, and support D-pad or stick menu navigation, controller-only high-score initials, Game Over restart with south button or Space, Game Over lobby exit with east button, Select, or Escape, north-button Achievements access, east-button or Select back, and Start-to-pause without duplicate scene polling. Bridge Player 1 directions plus face buttons to the six keyboard-owned replicas only while each scene is active.
- **Reliable Pause Restart** - Use Escape or controller Start to pause an active game, close an open utility panel with Escape before play resumes, prevent controller Start from pausing behind that panel, quit only through the explicit Pause menu action, and restart without losing the active difficulty, local co-op mode, or versus mode configuration.
...
- **Connected Score Swarm** - Exchange signer-fingerprinted Ed25519 score and replay claims across manually connected ARC1 peers, relay the newest 256 logical clocks, and retain up to 4,096 verified claims in an indexed store that restores through bounded verification batches.
