...
test('retain motion-safe contrast and bounded frame cadence under low-quality high load', async ({ page }) => {
  ...
  for (const target of scenes) {
    ...
    await expect.poll(() => page.evaluate(key => (window as any).game.scene.isActive(key), target.key)).toBe(true);

    if (target.dense) await page.evaluate(() => {
      const scene = (window as any).game.scene.getScene('DanmakuScene');
      scene.projectiles.clear();
      for (let index = 0; index < 512; index++) {
        const angle = index * 2.399963;
        scene.projectiles.spawn({ x: 320 + Math.cos(angle) * (90 + index % 130), y: 230 + Math.sin(angle) * (70 + index % 110), vx: Math.cos(angle) * 24, vy: Math.sin(angle) * 24, life: 4, kind: index % 4 });
      }
      scene.collide();
      if (scene.cameras.main.shakeEffect.isRunning) throw new Error('Reduced motion allowed camera shake');
    });
    ...
    const metrics = await page.evaluate(async () => {
      ...
      for (let offset = 0; offset < pixels.length; offset += 16) {
        ...
        if (luma < 36) dark++; if (luma > 118) bright++;
      }
      const ordered = deltas.slice(5).sort((a, b) => a - b);
      return { p95FrameMs: ordered[Math.ceil(ordered.length * .95) - 1], dark, bright };
    });
    expect(metrics.p95FrameMs, `${target.key} LOW-quality p95`).toBeLessThan(45);
    expect(metrics.dark, `${target.key} needs dark silhouette separation`).toBeGreaterThan(1000);
    expect(metrics.bright, `${target.key} needs bright silhouette separation`).toBeGreaterThan(40);
  }
});
