...
test('launch Neon Tactics within bounded swarm limits', async ({ page }) => {
  await page.goto('/'); await page.locator('#app canvas').first().waitFor(); await launch(page, 14, 'TacticsScene'); const count = await page.evaluate(() => ((window as typeof window & { game: { scene: { getScene(key: string): unknown } } }).game.scene.getScene('TacticsScene') as { count: number }).count); expect(count).toBeGreaterThan(100); expect(count).toBeLessThanOrEqual(1000);
});
