# Credits & Acknowledgments

**Universal Retro Arcade** is an original open-source project by the **BiosSystem Open Source Community**.

This project does not fork or derive from any external game codebase. All 11 game implementations are original BiosSystem creations built on top of the Phaser 3 framework.

---

## 🕹️ Core Runtime Dependencies

| Library | Author / Org | License |
|---|---|---|
| [Phaser 3](https://github.com/photonstorm/phaser) | Richard Davey (Photon Storm) & Phaser Contributors | MIT |
| [TypeScript](https://github.com/microsoft/TypeScript) | Microsoft | Apache 2.0 |
| [Vite](https://github.com/vitejs/vite) | Evan You & Vite Contributors | MIT |

## 🖥️ Desktop Runtime (Tauri)

| Library | Author / Org | License |
|---|---|---|
| [Tauri v2](https://github.com/tauri-apps/tauri) | Tauri Programme within the Commons Conservancy | MIT / Apache 2.0 |
| [tauri-plugin-log](https://github.com/tauri-apps/plugins-workspace) | Tauri Programme within the Commons Conservancy | MIT / Apache 2.0 |
| [Rust](https://www.rust-lang.org/) | The Rust Foundation | MIT / Apache 2.0 |
| [serde](https://github.com/serde-rs/serde) | David Tolnay | MIT / Apache 2.0 |
| [serde_json](https://github.com/serde-rs/json) | David Tolnay | MIT / Apache 2.0 |

## 🎨 Fonts

| Resource | Author / Org | License |
|---|---|---|
| [Share Tech Mono](https://fonts.google.com/specimen/Share+Tech+Mono) | Carrois Apostrophe / Google Fonts | OFL-1.1 |

---

## 🎮 BiosSystem Original Game Implementations

All 11 games in Universal Retro Arcade are original BiosSystem implementations. No game code is derived from or copied from any third-party source:

| Game | Genre |
|---|---|
| Snake Evolution | Snake / Growth |
| Neon Pong | Paddle |
| Astro Drift | Asteroids |
| Brick Breaker | Breakout |
| Froggie Crosser | Frogger-style |
| Space Defenders | Space Invaders-style |
| Tetris Pulse | Falling blocks |
| Minesweeper | Puzzle |
| Pixel Runner | Endless runner |
| Brave Bird | Flappy-style |
| Cyber Chasm | Platformer |

---

*Crafted with passion by BiosSystem - Open Source Community*
